...
| **Activation**  | Schools must actively opt in, either by providing their own API key or by subscribing to Tally's paid AI add-on, in which case Tally makes the requests using its own provider account. The feature is disabled by default.                                                                                      |

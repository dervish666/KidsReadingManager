...
- **This publishes live.** The skill commits, pushes and then deploys `public/reading-news.json` (Sam watches the output rather than gating each push) — so accuracy and appropriateness are non-negotiable (see vetting). **The push alone does NOT publish**: `public/` is copied into `build/`, and `build/` only reaches production via a deploy. Cloudflare Workers Builds used to redeploy on push, but it was disconnected on 2026-07-02 after it started deploying stale commits — so `npm run go` is the deploy step. See Step 5.
...
3. Commit and push. Stage **only** `public/reading-news.json`, leaving any unrelated working-tree changes alone:
   ```bash
   git add public/reading-news.json && git commit -m "content: refresh reading news" && git push
   ```
4. **Deploy** — the push does not publish on its own:

   ```bash
   npm run go
   ```

   This was wrong in an earlier version of this skill, which said the push auto-published and told you never to run `npm run go`. That was true only while Cloudflare Workers Builds was connected; it was disconnected on 2026-07-02 (see CLAUDE.md). The July 2026 issue is stamped `2026-07-02` and sat stale on the live site for six weeks as a result — the skill pushed, assumed, and nothing deployed.

5. **Confirm it actually went live**, comparing the date rather than eyeballing:

   ```bash
   curl -sL https://tallyreading.uk/reading-news.json | python3 -c "import json,sys; print(json.load(sys.stdin)['generatedAt'])"
   ```

   It must print today's date. A stale date here means the deploy did not happen — do not report success until it matches.

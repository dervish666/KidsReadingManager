...
- `parent_access_tokens` - Token-based parent portal access per student per academic year (token auth, teacher-revocable). Printed via `src/components/parent/QRCodeSheet.js`, which takes `scope="class"` (default) or `scope="school"`; the school scope calls `POST /api/parent/school/generate` then `GET /api/parent/school/tokens` and prints one class per page, unassigned pupils last. **Teacher-facing parent routes must be listed in `isPublicParentRoute()` in `src/worker.js`** — everything under `/api/parent/` is public by default, so a new prefix without an entry there gets no JWT and no `organizationId`
...
### Pupils' Own Books (books from home)

Children bring books in from home, and a teacher has to log against _something_. Before v3.135.0 the only way was "Add to Library", which put every home copy into the school's catalogue and, from there, into every other child's recommendations. Now a book can be linked to a school as a **pupil's own copy**, and the whole distinction rides on the long-vestigial `org_book_selections.is_available` column: `1` = the school holds it (its library), `0` = a pupil's own copy.

- **Every library read already filtered `is_available = 1`**, so a home copy is excluded from recommendations (AI and `computeLibraryRecommendations`), the parent portal's library search, metadata enrichment and the book counts with no new query logic. The two places that must _accept_ a `0` row are session validation (`routes/students/sessions.js`, both the single and bulk routes) and the parent portal's home-reading log; the lists behind the session autocomplete and the Books page (`GET /api/books?all=true`, minimal or full) return both kinds with a `fromHome` flag. `ORG_BOOK_SELECT` selects the column and `rowToBook` maps it, so it is `false` on any global read that did not select it.
- **The library wins on conflict.** `linkBookToOrg` in `routes/books/isbn.js` upserts with `MAX(is_available, excluded.is_available)`: scanning a book the school already holds as "pupil's own" changes nothing, and every "add to library" path (scan, import, `POST /api/books`) writes `1`, which is how a home copy becomes a library book when the school buys one. `PUT /api/books/:id/ownership { fromHome }` does the same explicitly in either direction; only the per-org link changes, never the shared `books` row, so it needs only `requireTeacher()`.
- **Three ways in, one flag.** The scan dialog offers "Pupil's Own Book" beside "Add to Library" (and, for a known home copy, "Select This Book" beside a promote-to-library button); the session autocomplete shows a small switch while the teacher is typing that routes the Enter and OpenLibrary paths through `findOrCreateBook(..., { fromHome })`; the Books page shows a "Pupil's own" chip, an "Owned by" filter and an "Add to library" button per row. CSV export excludes home copies.
- Removing a book from a school still hard-deletes the `org_book_selections` row, and the demo snapshot only ever writes `1`.
...
**The model picker is live, not curated.** `GET /api/settings/ai/models?provider=…` (`src/routes/settings/ai.js`) lists what the provider's own models API returns, newest first, using the first key it finds down the ladder (org key for that provider → platform key for that provider → env var) and says which in `source`. It is **not** gated on the add-on — the add-on gates spending, not names — because the old gate meant every school without it saw a hardcoded three-model list under a status line saying "Platform key". `STATIC_MODELS` in `src/components/AISettings.js` is the no-key-at-all fallback only; don't try to keep it current, keep it short.

**Only the low-cost tier is ever offered or run.** `src/utils/aiModelTiers.js` holds one id pattern per provider (Haiku, nano/mini, Flash) and a cheap default each. `fetchProviderModels` trims every live list to it before any picker sees it, and `resolveAiConfig` + `buildFailoverChain` clamp the stored preference to it at call time, with a `console.warn` when they had to. Two reasons it is enforced at call time and not just in the picker: most schools run on the platform key, where the model comes from `platform_ai_keys.model_preference` (the owner's Platform Settings pick), not the school's own picker — so the school-facing picker is cosmetic on that path — and a preference stored before the rule existed would otherwise run at frontier rates. Widening the tier (say, adding Sonnet) is a one-pattern change in that file; a Fable 5.1 recommendation call costs ten times a Haiku one for the same JSON.
...
- **Hourly at :07** — demo environment reset (`src/services/demoReset.js`). **:07, not :00** — on the hour it collided with the 02:00 and 03:00 nightly jobs. The schedule string lives in **three** places that must agree: `wrangler.toml` `[triggers]`, the `event.cron` test in `src/worker.js`, and `schedule.value` in that job's `Sentry.withMonitor` options; a mismatch is silently rejected at ingest and the monitor just stops working.
- **Hourly at :37** — cron liveness watchdog (`checkCronFreshness`). Moved off :07 in v3.135.2: when D1 shed load at 17:07 on 2026-09-16, the watchdog's own read failed and the reset behind it fell back to row-by-row, so one bad minute produced three Sentry issues. On its own minute the watchdog cannot be knocked over by the job it guards.
...
**Session dates slide forward on every reset.** The snapshot's reading sessions are fixed January–April 2026, and class goals key on the academic year (rolls over 1 August), so from August every demo class showed "0 of 30 reading days" and a garden nobody had touched. `sessionDateShiftDays()` in `src/services/demoReset.js` moves every session forward by a whole number of weeks so the newest lands in the last seven days (weekdays preserved, gaps preserved); a Phase 4 after the badge pass keeps 14 random ticker events spread over the last few hours and spreads `student_badges.earned_at` across eight weeks, because the badge pass otherwise stamps all ~190 awards at the same second and the Today tab reads "192 badges earned at 5:08 PM". If you re-export the snapshot, the exported dates will be whatever the last reset wrote, which is fine: the shift is relative to the newest session, not to a fixed origin.
...
**The class reconcile has to handle the September rollover.** A delta's class fetch is filtered by `updated_after`, so it can never be reconciled against — but the new academic year arrives _as_ a delta. On 2026-08-31 Cheddar Grove's 3 AM delta created 14 new classes and left last year's 16 active, and every class dropdown read "1P, 1P, 2A, 2A, 5D, 5D" until a full sync was run by hand. A delta that creates classes now re-fetches the unfiltered list and deactivates against that; a delta that creates none still touches nothing, and an empty fetch never deactivates anything. Manual classes (`wonde_class_id IS NULL`) are never touched.

**MyLogin OAuth2 SSO** (`src/routes/mylogin.js`): OAuth2 Authorization Code flow. Login initiation stores state in KV, redirects to MyLogin. Callback exchanges code for token, fetches user profile, matches org by `wonde_school_id`, creates/updates user by `mylogin_id`, issues standard Tally JWT. **Staff only:** the only permitted MyLogin types are admin→admin and employee→teacher (`STAFF_ROLE_BY_MYLOGIN_TYPE` in `src/routes/mylogin.js`). Any other type — student, parent, or one added by MyLogin later — is refused at the callback with `?auth=error&reason=staff_only` _before_ any org lookup or user row is touched, so no account is ever created for a pupil. Students previously mapped to `readonly`, which let a pupil see every class, student and teacher in their school (found by Wonde's integration testers, July 2026). Student _records_ still sync from Wonde into the `students` table as normal — that is unrelated to logins.

**MyLogin's own quirks, verified against their live API (v3.117.0):** their profile schema makes `email` **nullable** (support staff often have none in the MIS) and `organisation.wonde_id` **nullable** (org not Wonde-connected) — `users.email` is `UNIQUE NOT NULL` _globally_, so the callback synthesises `mylogin-<id>@no-email.invalid` on insert, `COALESCE(?, email)` on update, and adopts a same-org local account that already holds the address rather than colliding (cross-org or already-linked → `email_conflict`). The stored row's `organization_id` is the tenant boundary the JWT is scoped to, so it is re-checked against the org MyLogin authenticated for (`school_mismatch`) — matching by `mylogin_id` alone would serve a moved teacher their previous school's children. `/oauth/authorize` also takes an optional `organisation=<slug|encoded id>` that skips MyLogin's national school picker; we forward it from `/?school=<slug>`. To check whether a school is on MyLogin at all (separate from Wonde data approval): `GET https://app.mylogin.com/login/api/organisations?search=<term>`.

**Year group comes from the `year` relationship, not `education_details`.** `education_details.data.current_nc_year` is the documented field and every Wonde connection Tally has ever had leaves it empty, so all 2,393 pupils across the three connected schools had a NULL year group. The `year` include (already requested, never read until v3.125.0) carries the real code for 397 of Cheddar Grove's 424 pupils, nursery `N1`/`N2` included. `mapWondeStudent` prefers `current_nc_year`, falls back to `year.data.code`, and treats a blank string as missing. Resolution precedence downstream is unchanged: `students.year_group` → `classes.year_group` → parsed from the class name (`src/utils/yearGroup.js`).
...
- **A missing `/static/*` asset must 404, not fall back to index.html**: `not_found_handling = "single-page-application"` applies to every path, and `public/_headers` matches on request path rather than on what was served, so a since-replaced bundle came back as `200 text/html` stamped `max-age=31536000, immutable`. The browser asked for CSS, got HTML, and with `nosniff` refused it, which is the "Loading CSS chunk N failed" family. `/static/*` now runs through the Worker (`run_worker_first` + the `ASSETS` binding) purely so that branch can answer a real 404 with `no-store`; it adds no other headers, because `_headers` still applies through the binding (verified locally 2026-09-14) and a second header source in the Worker is what drifted into a weaker CSP last time.
- **A chunk-load error is usually a stale tab, not a broken deploy**: every route is a `React.lazy` import, so a tab opened before a deploy points at bundles that are no longer served. `src/utils/chunkRecovery.js` reloads once (30s sessionStorage cooldown, skipped for crawlers) and index.html is `no-cache`, so the reload lands on the current bundle. A second failure inside the cooldown falls through to the error UI and Sentry, which is where a genuinely bad chunk belongs.
- **Crawler errors are not incidents**: the landing page is a lazy chunk, so a bot that starts fetching it and gives up files a plausible-looking chunk error against a URL that is live and serving 200 (TALLY-READING-J, bingbot, 13 Sep 2026). `isCrawlerUserAgent` in `src/utils/sentryFilter.js` drops those events; Sentry's own "Filter out known web crawlers" inbound filter is worth turning on as well.
...
- **MyLogin may never remove an elevated Tally role**: `resolveRole` in `src/routes/mylogin.js` blocks elevation _and_ pins `admin`/`owner`. Users are matched by `mylogin_id`, then email, then `wonde_employee_id`. See [Incidents](docs/incidents.md#mylogin-role-pin).
...
- **Dependabot**: weekly npm + monthly actions updates; dev and prod minor/patch grouped, majors individual. Read advisories for _applicability_ first — most land on dev-only transitives that never reach the Worker. See [Incidents](docs/incidents.md#dependabot-scope).

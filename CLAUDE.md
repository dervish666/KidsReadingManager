...
- `APP_VERSION` - Injected at deploy time by `npm run deploy` (`--var APP_VERSION:$npm_package_version`), not set in `wrangler.toml`. Tags Worker errors with the release so it matches the frontend's. Falls back to `dev` locally.

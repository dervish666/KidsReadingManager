/**
 * Core organization CRUD: current org, owner-wide list, stats, get by id,
 * create, update and soft-delete.
 *
 * Split out of routes/organization.js so that file is a pure aggregator,
 * matching routes/auth.js and routes/settings.js. Compliance endpoints
 * (audit log, DPA consent, purge) are a sibling in this directory; mounting
 * order lives in the parent.
 */
 ...
 * Requires: admin role
 */
coreRouter.get('/all', requireAdmin(), async (c) => {
  try {
    const db = getDB(c.env);
 ...
 */
coreRouter.get('/stats', requireReadonly(), async (c) => {
  try {
    const db = getDB(c.env);
    const organizationId = c.get('organizationId');
 ...
 * Requires: owner role
 */
coreRouter.get('/:id', requireOwner(), async (c) => {
  try {
    const db = getDB(c.env);
 ...
 * }
 */
coreRouter.post('/create', requireOwner(), auditLog('create', 'organization'), async (c) => {
  try {
    const db = getDB(c.env);
 ...
 * }
 */
coreRouter.put('/:id', requireOwner(), auditLog('update', 'organization'), async (c) => {
  try {
    const db = getDB(c.env);
    const orgId = c.req.param('id');
    const body = await c.req.json();
 ...
 * Requires: owner role
 */
coreRouter.delete('/:id', requireOwner(), auditLog('delete', 'organization'), async (c) => {
  try {
    const db = getDB(c.env);
    const orgId = c.req.param('id');
 ...
 * }
 */
coreRouter.put('/', requireOwner(), auditLog('update', 'organization'), async (c) => {
  try {
    const db = getDB(c.env);
    const organizationId = c.get('organizationId');
    const body = await c.req.json();

/**
 * Core user CRUD: list, get, create, update, delete, and admin password reset.
 *
 * Split out of routes/users.js so that file is a pure aggregator, matching
 * routes/auth.js and routes/settings.js. The GDPR and class-assignment
 * endpoints are siblings in this directory; mounting order lives in the
 * parent.
 */
 ...
 * Requires: admin role
 */
coreRouter.get('/', requireAdmin(), async (c) => {
  try {
 ...
 * }
 */
coreRouter.put('/:id', auditLog('update', 'user'), async (c) => {
  try {
    const db = getDB(c.env);
    const currentUserOrgId = c.get('organizationId');
 ...
 * Requires: admin role
 */
coreRouter.delete('/:id', requireAdmin(), auditLog('delete', 'user'), async (c) => {
  try {
    const db = getDB(c.env);
    const organizationId = c.get('organizationId');
    const currentUserId = c.get('userId');
 ...
 * Requires: admin role
 */
coreRouter.post('/:id/reset-password', requireAdmin(), auditLog('update', 'user'), async (c) => {
  try {
    const db = getDB(c.env);
    const organizationId = c.get('organizationId');
    const targetUserId = c.req.param('id');

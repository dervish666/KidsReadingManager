/**
 * Core student CRUD: list, get, create, update, delete, plus the
 * current-book, feedback and bulk baseline-reads endpoints.
 *
 * Split out of routes/students.js so that file is a pure aggregator, matching
 * routes/auth.js and routes/settings.js. Sessions, stats, AI summary, streaks,
 * bulk import and GDPR are siblings in this directory; mounting order lives in
 * the parent.
 */
 ...
 */
coreRouter.post('/', auditLog('create', 'student'), async (c) => {
 ...
 */
coreRouter.put('/baseline-reads/bulk', requireTeacher(), async (c) => {
  const db = getDB(c.env);
  const organizationId = c.get('organizationId');
...
coreRouter.put('/:id', auditLog('update', 'student'), async (c) => {
  const { id } = c.req.param();
  const body = await c.req.json();
 ...
 * a hard delete.
 */
coreRouter.delete('/:id', auditLog('delete', 'student'), async (c) => {
  const { id } = c.req.param();
 ...
 * book-edit popover.
 */
coreRouter.put('/:id/current-book', requireTeacher(), async (c) => {
  const { id } = c.req.param();
  const body = await c.req.json();
 ...
 */
coreRouter.put('/:id/feedback', requireTeacher(), async (c) => {
  const { id } = c.req.param();
  const body = await c.req.json();

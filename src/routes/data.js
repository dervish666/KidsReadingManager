...
// Import services
import { getData, replaceData } from '../services/kvService';
...
dataRouter.use('/*', async (c, next) => {
  if (c.env.JWT_SECRET) {
    return c.json(
      {
        error:
          'Data export/import is not available in multi-tenant mode. Use organization-specific endpoints instead.',
      },
      403
    );
  }

...
export async function fetchMetadata(book, apiKey) {
  ...
  try {
    ...
    try {
      ...
      // Best-effort parse of cached metadata — ignore malformed cache entries.
    }
    ...
    console.warn(
      `[metadata] provider 'hardcover' failed, returning empty: ${err instanceof Error ? err.message : String(err)}`
    );
    return empty;
  }
}

 ...
 *
 * Atomically cascade-deletes all org-scoped data across 29 tables in FK-safe
...
const DELETE_ORDER = [
  ...
  { table: 'ticker_events', where: `organization_id = ?` },
  {
...
export async function hardDeleteOrganization(db, orgId, env = null) {
  ...
  // 3. Execute atomically. D1's 100-statement batch limit is well above our 32.
  try {
  ...
  const tablesProcessed = DELETE_ORDER.length + 1; // 29 + data_rights_log cleanup

 ...
 * Email/password with organization context, plus MyLogin SSO. The legacy
 * shared-password form was removed with the legacy auth mode (2026-08).
 */
// API URL - relative path since frontend and API are served from the same origin
const API_URL = '/api';

const Login = ({ onBackToLanding } = {}) => {
  const { loginWithEmail, apiError, serverAuthModeDetected, ssoEnabled } = useAuth();
  ...
  const handleSubmit = (event) => handleMultiTenantLogin(event);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {view === 'login' && renderMultiTenantForm()}

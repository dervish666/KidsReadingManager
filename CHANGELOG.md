...
- **ESLint** — ESLint 10 with flat config, `eslint-plugin-react-hooks`, and environment-aware globals (browser for frontend, Cloudflare Workers for backend, Node for jsonProvider, Vitest for tests). `npm run lint` / `npm run lint:fix` scripts added. Zero errors on initial run; 116 warnings (mostly unused vars in tests).

### Fixed

- **`trialOrg` scoping bug in stripeWebhook.js** — variable was declared inside `try` block but referenced in `catch` block, causing a ReferenceError if the DB query failed. Hoisted to `let` before `try`.
- **`PageFallback` component created during render in App.js** — inline component definition caused React to remount on every render. Moved to module scope.
- **`hasOwnProperty` prototype access in stats.js** — replaced `obj.hasOwnProperty()` with `Object.hasOwn()` to prevent prototype pollution risk.
- **Dead initial assignments** in `aiService.js`, `studentProfile.js`, `BookManager.js` — variables initialised then immediately overwritten in all branches.
